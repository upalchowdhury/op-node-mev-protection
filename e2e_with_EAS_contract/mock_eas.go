// Code generated - DO NOT EDIT.
// This file is a generated binding and any manual changes will be lost.

package contract

import (
	"errors"
	"math/big"
	"strings"

	ethereum "github.com/ethereum/go-ethereum"
	"github.com/ethereum/go-ethereum/accounts/abi"
	"github.com/ethereum/go-ethereum/accounts/abi/bind"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/core/types"
	"github.com/ethereum/go-ethereum/event"
)

// Reference imports to suppress errors if they are not otherwise used.
var (
	_ = errors.New
	_ = big.NewInt
	_ = strings.NewReader
	_ = ethereum.NotFound
	_ = bind.Bind
	_ = common.Big1
	_ = types.BloomLookup
	_ = event.NewSubscription
	_ = abi.ConvertType
)

// MockEASMetaData contains all meta data concerning the MockEAS contract.
var MockEASMetaData = &bind.MetaData{
	ABI: "[{\"anonymous\":false,\"inputs\":[{\"indexed\":false,\"internalType\":\"string\",\"name\":\"txHash\",\"type\":\"string\"},{\"indexed\":true,\"internalType\":\"address\",\"name\":\"attester\",\"type\":\"address\"}],\"name\":\"AttestationCreated\",\"type\":\"event\"},{\"inputs\":[{\"internalType\":\"string\",\"name\":\"txHash\",\"type\":\"string\"},{\"internalType\":\"string\",\"name\":\"encryptedData\",\"type\":\"string\"}],\"name\":\"attestTransactionWithPrivateData\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"string\",\"name\":\"\",\"type\":\"string\"}],\"name\":\"attestations\",\"outputs\":[{\"internalType\":\"string\",\"name\":\"txHash\",\"type\":\"string\"},{\"internalType\":\"string\",\"name\":\"encryptedData\",\"type\":\"string\"},{\"internalType\":\"address\",\"name\":\"attester\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"string\",\"name\":\"txHash\",\"type\":\"string\"}],\"name\":\"getAttestation\",\"outputs\":[{\"internalType\":\"string\",\"name\":\"\",\"type\":\"string\"},{\"internalType\":\"string\",\"name\":\"\",\"type\":\"string\"},{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"string\",\"name\":\"txHash\",\"type\":\"string\"},{\"internalType\":\"string\",\"name\":\"encryptedData\",\"type\":\"string\"}],\"name\":\"verifyAttestation\",\"outputs\":[{\"internalType\":\"bool\",\"name\":\"\",\"type\":\"bool\"}],\"stateMutability\":\"view\",\"type\":\"function\"}]",
	Bin: "",
}

// MockEASABI is the input ABI used to generate the binding from.
// Deprecated: Use MockEASMetaData.ABI instead.
var MockEASABI = MockEASMetaData.ABI

// MockEASBin is the compiled bytecode used for deploying new contracts.
// Deprecated: Use MockEASMetaData.Bin instead.
var MockEASBin = MockEASMetaData.Bin

// DeployMockEAS deploys a new Ethereum contract, binding an instance of MockEAS to it.
func DeployMockEAS(auth *bind.TransactOpts, backend bind.ContractBackend) (common.Address, *types.Transaction, *MockEAS, error) {
	parsed, err := MockEASMetaData.GetAbi()
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	if parsed == nil {
		return common.Address{}, nil, nil, errors.New("GetABI returned nil")
	}

	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(MockEASBin), backend)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	return address, tx, &MockEAS{MockEASCaller: MockEASCaller{contract: contract}, MockEASTransactor: MockEASTransactor{contract: contract}, MockEASFilterer: MockEASFilterer{contract: contract}}, nil
}

// MockEAS is an auto generated Go binding around an Ethereum contract.
type MockEAS struct {
	MockEASCaller     // Read-only binding to the contract
	MockEASTransactor // Write-only binding to the contract
	MockEASFilterer   // Log filterer for contract events
}

// MockEASCaller is an auto generated read-only Go binding around an Ethereum contract.
type MockEASCaller struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// MockEASTransactor is an auto generated write-only Go binding around an Ethereum contract.
type MockEASTransactor struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// MockEASFilterer is an auto generated log filtering Go binding around an Ethereum contract events.
type MockEASFilterer struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// MockEASSession is an auto generated Go binding around an Ethereum contract,
// with pre-set call and transact options.
type MockEASSession struct {
	Contract     *MockEAS          // Generic contract binding to set the session for
	CallOpts     bind.CallOpts     // Call options to use throughout this session
	TransactOpts bind.TransactOpts // Transaction auth options to use throughout this session
}

// MockEASCallerSession is an auto generated read-only Go binding around an Ethereum contract,
// with pre-set call options.
type MockEASCallerSession struct {
	Contract *MockEASCaller // Generic contract caller binding to set the session for
	CallOpts bind.CallOpts  // Call options to use throughout this session
}

// MockEASTransactorSession is an auto generated write-only Go binding around an Ethereum contract,
// with pre-set transact options.
type MockEASTransactorSession struct {
	Contract     *MockEASTransactor // Generic contract transactor binding to set the session for
	TransactOpts bind.TransactOpts  // Transaction auth options to use throughout this session
}

// MockEASRaw is an auto generated low-level Go binding around an Ethereum contract.
type MockEASRaw struct {
	Contract *MockEAS // Generic contract binding to access the raw methods on
}

// MockEASCallerRaw is an auto generated low-level read-only Go binding around an Ethereum contract.
type MockEASCallerRaw struct {
	Contract *MockEASCaller // Generic read-only contract binding to access the raw methods on
}

// MockEASTransactorRaw is an auto generated low-level write-only Go binding around an Ethereum contract.
type MockEASTransactorRaw struct {
	Contract *MockEASTransactor // Generic write-only contract binding to access the raw methods on
}

// NewMockEAS creates a new instance of MockEAS, bound to a specific deployed contract.
func NewMockEAS(address common.Address, backend bind.ContractBackend) (*MockEAS, error) {
	contract, err := bindMockEAS(address, backend, backend, backend)
	if err != nil {
		return nil, err
	}
	return &MockEAS{MockEASCaller: MockEASCaller{contract: contract}, MockEASTransactor: MockEASTransactor{contract: contract}, MockEASFilterer: MockEASFilterer{contract: contract}}, nil
}

// NewMockEASCaller creates a new read-only instance of MockEAS, bound to a specific deployed contract.
func NewMockEASCaller(address common.Address, caller bind.ContractCaller) (*MockEASCaller, error) {
	contract, err := bindMockEAS(address, caller, nil, nil)
	if err != nil {
		return nil, err
	}
	return &MockEASCaller{contract: contract}, nil
}

// NewMockEASTransactor creates a new write-only instance of MockEAS, bound to a specific deployed contract.
func NewMockEASTransactor(address common.Address, transactor bind.ContractTransactor) (*MockEASTransactor, error) {
	contract, err := bindMockEAS(address, nil, transactor, nil)
	if err != nil {
		return nil, err
	}
	return &MockEASTransactor{contract: contract}, nil
}

// NewMockEASFilterer creates a new log filterer instance of MockEAS, bound to a specific deployed contract.
func NewMockEASFilterer(address common.Address, filterer bind.ContractFilterer) (*MockEASFilterer, error) {
	contract, err := bindMockEAS(address, nil, nil, filterer)
	if err != nil {
		return nil, err
	}
	return &MockEASFilterer{contract: contract}, nil
}

// bindMockEAS binds a generic wrapper to an already deployed contract.
func bindMockEAS(address common.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
	parsed, err := MockEASMetaData.GetAbi()
	if err != nil {
		return nil, err
	}
	return bind.NewBoundContract(address, *parsed, caller, transactor, filterer), nil
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_MockEAS *MockEASRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _MockEAS.Contract.MockEASCaller.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_MockEAS *MockEASRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _MockEAS.Contract.MockEASTransactor.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_MockEAS *MockEASRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _MockEAS.Contract.MockEASTransactor.contract.Transact(opts, method, params...)
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_MockEAS *MockEASCallerRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _MockEAS.Contract.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_MockEAS *MockEASTransactorRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _MockEAS.Contract.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_MockEAS *MockEASTransactorRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _MockEAS.Contract.contract.Transact(opts, method, params...)
}

// Attestations is a free data retrieval call binding the contract method 0x249d026b.
//
// Solidity: function attestations(string ) view returns(string txHash, string encryptedData, address attester)
func (_MockEAS *MockEASCaller) Attestations(opts *bind.CallOpts, arg0 string) (struct {
	TxHash        string
	EncryptedData string
	Attester      common.Address
}, error) {
	var out []interface{}
	err := _MockEAS.contract.Call(opts, &out, "attestations", arg0)

	outstruct := new(struct {
		TxHash        string
		EncryptedData string
		Attester      common.Address
	})
	if err != nil {
		return *outstruct, err
	}

	outstruct.TxHash = *abi.ConvertType(out[0], new(string)).(*string)
	outstruct.EncryptedData = *abi.ConvertType(out[1], new(string)).(*string)
	outstruct.Attester = *abi.ConvertType(out[2], new(common.Address)).(*common.Address)

	return *outstruct, err

}

// Attestations is a free data retrieval call binding the contract method 0x249d026b.
//
// Solidity: function attestations(string ) view returns(string txHash, string encryptedData, address attester)
func (_MockEAS *MockEASSession) Attestations(arg0 string) (struct {
	TxHash        string
	EncryptedData string
	Attester      common.Address
}, error) {
	return _MockEAS.Contract.Attestations(&_MockEAS.CallOpts, arg0)
}

// Attestations is a free data retrieval call binding the contract method 0x249d026b.
//
// Solidity: function attestations(string ) view returns(string txHash, string encryptedData, address attester)
func (_MockEAS *MockEASCallerSession) Attestations(arg0 string) (struct {
	TxHash        string
	EncryptedData string
	Attester      common.Address
}, error) {
	return _MockEAS.Contract.Attestations(&_MockEAS.CallOpts, arg0)
}

// GetAttestation is a free data retrieval call binding the contract method 0xd607daf6.
//
// Solidity: function getAttestation(string txHash) view returns(string, string, address)
func (_MockEAS *MockEASCaller) GetAttestation(opts *bind.CallOpts, txHash string) (string, string, common.Address, error) {
	var out []interface{}
	err := _MockEAS.contract.Call(opts, &out, "getAttestation", txHash)

	if err != nil {
		return *new(string), *new(string), *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(string)).(*string)
	out1 := *abi.ConvertType(out[1], new(string)).(*string)
	out2 := *abi.ConvertType(out[2], new(common.Address)).(*common.Address)

	return out0, out1, out2, err

}

// GetAttestation is a free data retrieval call binding the contract method 0xd607daf6.
//
// Solidity: function getAttestation(string txHash) view returns(string, string, address)
func (_MockEAS *MockEASSession) GetAttestation(txHash string) (string, string, common.Address, error) {
	return _MockEAS.Contract.GetAttestation(&_MockEAS.CallOpts, txHash)
}

// GetAttestation is a free data retrieval call binding the contract method 0xd607daf6.
//
// Solidity: function getAttestation(string txHash) view returns(string, string, address)
func (_MockEAS *MockEASCallerSession) GetAttestation(txHash string) (string, string, common.Address, error) {
	return _MockEAS.Contract.GetAttestation(&_MockEAS.CallOpts, txHash)
}

// VerifyAttestation is a free data retrieval call binding the contract method 0x06dda2d3.
//
// Solidity: function verifyAttestation(string txHash, string encryptedData) view returns(bool)
func (_MockEAS *MockEASCaller) VerifyAttestation(opts *bind.CallOpts, txHash string, encryptedData string) (bool, error) {
	var out []interface{}
	err := _MockEAS.contract.Call(opts, &out, "verifyAttestation", txHash, encryptedData)

	if err != nil {
		return *new(bool), err
	}

	out0 := *abi.ConvertType(out[0], new(bool)).(*bool)

	return out0, err

}

// VerifyAttestation is a free data retrieval call binding the contract method 0x06dda2d3.
//
// Solidity: function verifyAttestation(string txHash, string encryptedData) view returns(bool)
func (_MockEAS *MockEASSession) VerifyAttestation(txHash string, encryptedData string) (bool, error) {
	return _MockEAS.Contract.VerifyAttestation(&_MockEAS.CallOpts, txHash, encryptedData)
}

// VerifyAttestation is a free data retrieval call binding the contract method 0x06dda2d3.
//
// Solidity: function verifyAttestation(string txHash, string encryptedData) view returns(bool)
func (_MockEAS *MockEASCallerSession) VerifyAttestation(txHash string, encryptedData string) (bool, error) {
	return _MockEAS.Contract.VerifyAttestation(&_MockEAS.CallOpts, txHash, encryptedData)
}

// AttestTransactionWithPrivateData is a paid mutator transaction binding the contract method 0x8d5b8361.
//
// Solidity: function attestTransactionWithPrivateData(string txHash, string encryptedData) returns()
func (_MockEAS *MockEASTransactor) AttestTransactionWithPrivateData(opts *bind.TransactOpts, txHash string, encryptedData string) (*types.Transaction, error) {
	return _MockEAS.contract.Transact(opts, "attestTransactionWithPrivateData", txHash, encryptedData)
}

// AttestTransactionWithPrivateData is a paid mutator transaction binding the contract method 0x8d5b8361.
//
// Solidity: function attestTransactionWithPrivateData(string txHash, string encryptedData) returns()
func (_MockEAS *MockEASSession) AttestTransactionWithPrivateData(txHash string, encryptedData string) (*types.Transaction, error) {
	return _MockEAS.Contract.AttestTransactionWithPrivateData(&_MockEAS.TransactOpts, txHash, encryptedData)
}

// AttestTransactionWithPrivateData is a paid mutator transaction binding the contract method 0x8d5b8361.
//
// Solidity: function attestTransactionWithPrivateData(string txHash, string encryptedData) returns()
func (_MockEAS *MockEASTransactorSession) AttestTransactionWithPrivateData(txHash string, encryptedData string) (*types.Transaction, error) {
	return _MockEAS.Contract.AttestTransactionWithPrivateData(&_MockEAS.TransactOpts, txHash, encryptedData)
}

// MockEASAttestationCreatedIterator is returned from FilterAttestationCreated and is used to iterate over the raw logs and unpacked data for AttestationCreated events raised by the MockEAS contract.
type MockEASAttestationCreatedIterator struct {
	Event *MockEASAttestationCreated // Event containing the contract specifics and raw log

	contract *bind.BoundContract // Generic contract to use for unpacking event data
	event    string              // Event name to use for unpacking event data

	logs chan types.Log        // Log channel receiving the found contract events
	sub  ethereum.Subscription // Subscription for errors, completion and termination
	done bool                  // Whether the subscription completed delivering logs
	fail error                 // Occurred error to stop iteration
}

// Next advances the iterator to the subsequent event, returning whether there
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *MockEASAttestationCreatedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	// If the iterator completed, deliver directly whatever's available
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(MockEASAttestationCreated)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
			it.Event.Raw = log
			return true

		default:
			return false
		}
	}
	// Iterator still in progress, wait for either a data or an error event
	select {
	case log := <-it.logs:
		it.Event = new(MockEASAttestationCreated)
		if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
			it.fail = err
			return false
		}
		it.Event.Raw = log
		return true

	case err := <-it.sub.Err():
		it.done = true
		it.fail = err
		return it.Next()
	}
}

// Error returns any retrieval or parsing error occurred during filtering.
func (it *MockEASAttestationCreatedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *MockEASAttestationCreatedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// MockEASAttestationCreated represents a AttestationCreated event raised by the MockEAS contract.
type MockEASAttestationCreated struct {
	TxHash   string
	Attester common.Address
	Raw      types.Log // Blockchain specific contextual infos
}

// FilterAttestationCreated is a free log retrieval operation binding the contract event 0x121ed3452c21306e8ea9b632f37c9aaf822b11f2a6aa980480124ba8b6eb0ac8.
//
// Solidity: event AttestationCreated(string txHash, address indexed attester)
func (_MockEAS *MockEASFilterer) FilterAttestationCreated(opts *bind.FilterOpts, attester []common.Address) (*MockEASAttestationCreatedIterator, error) {

	var attesterRule []interface{}
	for _, attesterItem := range attester {
		attesterRule = append(attesterRule, attesterItem)
	}

	logs, sub, err := _MockEAS.contract.FilterLogs(opts, "AttestationCreated", attesterRule)
	if err != nil {
		return nil, err
	}
	return &MockEASAttestationCreatedIterator{contract: _MockEAS.contract, event: "AttestationCreated", logs: logs, sub: sub}, nil
}

// WatchAttestationCreated is a free log subscription operation binding the contract event 0x121ed3452c21306e8ea9b632f37c9aaf822b11f2a6aa980480124ba8b6eb0ac8.
//
// Solidity: event AttestationCreated(string txHash, address indexed attester)
func (_MockEAS *MockEASFilterer) WatchAttestationCreated(opts *bind.WatchOpts, sink chan<- *MockEASAttestationCreated, attester []common.Address) (event.Subscription, error) {

	var attesterRule []interface{}
	for _, attesterItem := range attester {
		attesterRule = append(attesterRule, attesterItem)
	}

	logs, sub, err := _MockEAS.contract.WatchLogs(opts, "AttestationCreated", attesterRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		defer sub.Unsubscribe()
		for {
			select {
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(MockEASAttestationCreated)
				if err := _MockEAS.contract.UnpackLog(event, "AttestationCreated", log); err != nil {
					return err
				}
				event.Raw = log

				select {
				case sink <- event:
				case err := <-sub.Err():
					return err
				case <-quit:
					return nil
				}
			case err := <-sub.Err():
				return err
			case <-quit:
				return nil
			}
		}
	}), nil
}

// ParseAttestationCreated is a log parse operation binding the contract event 0x121ed3452c21306e8ea9b632f37c9aaf822b11f2a6aa980480124ba8b6eb0ac8.
//
// Solidity: event AttestationCreated(string txHash, address indexed attester)
func (_MockEAS *MockEASFilterer) ParseAttestationCreated(log types.Log) (*MockEASAttestationCreated, error) {
	event := new(MockEASAttestationCreated)
	if err := _MockEAS.contract.UnpackLog(event, "AttestationCreated", log); err != nil {
		return nil, err
	}
	event.Raw = log
	return event, nil
}
